...
# =================== DIBR IMPORTS (for mask alignment) ===================
try:
    import sys
    from pathlib import Path
    # Try to import DIBR modules from parent directory
    dibr_path = Path(__file__).parent.parent / "DIBR-GS"
    if dibr_path.exists():
        sys.path.insert(0, str(dibr_path))
        from dibr import DIBRMasked
        DIBR_AVAILABLE = True
    else:
        print("[WARNING] DIBR-GS path not found. Falling back to source mask without target alignment.")
        DIBR_AVAILABLE = False
except ImportError as e:
    print(f"[WARNING] Failed to import DIBR modules: {e}")
    print("         Falling back to source mask without target alignment.")
    DIBR_AVAILABLE = False
# =========================================================================
...
def compute_dibr_target_mask(frame, depth, mask, K, w2c_src, w2c_tgt, backend='posewarp'):
    """
    计算DIBR在目标视角的动态区掩码边界。
    
    Args:
        frame: numpy array (H, W, 3) 源帧
        depth: numpy array (H, W) 源深度
        mask: numpy array (H, W) 源动态掩码
        K: numpy array (3, 3) 内参
        w2c_src: numpy array (3, 4) 源相机外参 (通常为Identity)
        w2c_tgt: numpy array (3, 4) 目标相机外参
        backend: str 'posewarp' 或 'backward'
    
    Returns:
        target_mask: torch tensor (1, 1, H, W) 目标视角的动态掩码 [0, 1]
    """
    if not DIBR_AVAILABLE:
        # Fallback: 返回源mask转为tensor
    ...
    try:
        ...
        if isinstance(w2c_src, torch.Tensor):
            w2c_src_np = w2c_src.cpu().numpy()
        else:
            w2c_src_np = w2c_src
            
        if isinstance(w2c_tgt, torch.Tensor):
            w2c_tgt_np = w2c_tgt.cpu().numpy()
        else:
            w2c_tgt_np = w2c_tgt
...
def scene_reconstruction(
    ...
    for iteration in range(first_iter, final_iter + 1):
        ...
        motion_masks = []
        dibr_target_masks = []  # =================== NEW: DIBR target masks ===================
        d_alphas = []
        d_depths = []
        ...
        for n_batch, viewpoint_cam in enumerate(viewpoint_cams):
            ...
            if stage != "warm":
                ...
                if stage == "fine" and DIBR_AVAILABLE:
                    ...
                    dibr_target_masks.append(dibr_mask)
                else:
                    # Fallback: use source mask
                    dibr_target_masks.append(viewpoint_cam.mask.unsqueeze(0))
                # =====================================================================
        ...
        if stage != "warm":
            ...
            # =================== NEW: Use DIBR target mask instead of source mask ===================
            if stage == "fine" and dibr_target_masks:
                dibr_target_mask_tensor = torch.cat(dibr_target_masks, 0)
                # Ensure shape compatibility with d_alpha_tensor
                if dibr_target_mask_tensor.dim() == 4 and dibr_target_mask_tensor.shape[1] == 1:
                    dibr_target_mask_tensor = dibr_target_mask_tensor.squeeze(1)  # Remove channel dim if needed
            else:
                dibr_target_mask_tensor = motion_mask_tensor
            # ====================================================================================
